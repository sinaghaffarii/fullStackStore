...
export default function LoginPage() {
  ...
  const handleMobileSubmit = (data: LoginInput) => {
    login(data, {
      onSuccess: () => {
  ...
  const handleOTPSubmit = (data: { code: string }) => {
    verifyOTP({ mobile, code: data.code });
  };
      ...
      <div className="relative w-full max-w-[420px] rounded-lg border border-gray-100 bg-white p-8 shadow-[0_8px_40px_-12px_rgba(0,0,0,0.1)] md:p-10">
        <div className="mb-8 text-center">
          ...
          <h1 className="mb-2 text-2xl font-bold tracking-tight text-gray-900">
            {step === 'mobile' ? 'خوش‌آمدید' : 'تأیید شماره همراه'}
          </h1>
        ...
        <div className="relative">
          {step === 'mobile' ? (
            <MobileForm isLoading={isLoggingIn} onSubmit={handleMobileSubmit} />
          ) : (
            <OTPForm
